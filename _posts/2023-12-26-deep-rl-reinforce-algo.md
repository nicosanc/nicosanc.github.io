...
**Objective** - The Objective Function is the function we want to maximize. It can be understood in many instances as the expected value of the Return function over many, many episodes. A basic formulation of the Objective Function is $$J(\pi_\theta) = \mathrm{E}_{\tau \sim \pi_\theta} \left[ \sum_{t=0}^{T} R_t \right]
